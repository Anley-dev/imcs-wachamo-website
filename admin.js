...
import { getFirestore, collection, addDoc, query, orderBy, onSnapshot, serverTimestamp, deleteDoc, doc, updateDoc } from "https://www.gstatic.com/firebasejs/12.13.0/firebase-firestore.js";
...
// Publish Post Function - Writes to "news" collection
window.publishPost = async function() {
    ...
    if (!title || !content) {
        messageDiv.innerHTML = "<span style='color:#ef4444;'>❌ Please fill both Title and Content</span>";
        return;
    }
    ...
    try {
        ...
        messageDiv.innerHTML = "<span style='color:#10b981;'>✅ Post Published Successfully!</span>";
...
function loadPosts() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach((doc) => {
            ...
            html += `
                <div class="post-item" style="margin-bottom: 15px; padding: 15px; border-left: 4px solid #10b981; background: #f9fafb; border-radius: 8px;">
                    <h4 style="margin: 0 0 5px 0;">${escapeHtml(post.title)}</h4>
                    <small style="display:block; color:#6b7280; margin:6px 0;">${date}</small>
                    <p style="margin: 10px 0; color: #374151;">${escapeHtml(post.content)}</p>
                    <div style="display: flex; gap: 5px;">
                        <button onclick="editPost('${doc.id}')" style="background: #f59e0b; color: white; padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer;">
                            ✏️ Edit
                        </button>
                        <button onclick="deletePost('${doc.id}')" style="background: #ef4444; color: white; padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer;">
                            🗑️ Delete
                        </button>
                    </div>
                </div>
        ...
        container.innerHTML = `<p style="color: #ef4444;">Error loading posts. Check console.</p>`;
    });
}
...
window.createEvent = async function() {
    ...
    if (!title || !date || !description) {
        messageDiv.innerHTML = "<span style='color:#ef4444;'>❌ Please fill all required fields</span>";
        return;
    }
    ...
    try {
        ...
        messageDiv.innerHTML = "<span style='color:#10b981;'>✅ Event created successfully!</span>";
...
function loadEvents() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach((doc) => {
            ...
            html += `
                <div class="event-item" style="margin-bottom: 15px; padding: 15px; border-left: 4px solid #3b82f6; background: #f0f9ff; border-radius: 8px;">
                    <h4 style="margin: 0 0 5px 0;">${escapeHtml(event.title)}</h4>
                    <small style="display:block; color:#6b7280; margin:6px 0;">📅 ${event.date}${displayTime}</small>
                    ...
                    <div style="display: flex; gap: 5px; margin-top: 10px;">
                        <button onclick="editEvent('${doc.id}')" style="background: #f59e0b; color: white; padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer;">
                            ✏️ Edit
                        </button>
                        <button onclick="deleteEvent('${doc.id}')" style="background: #ef4444; color: white; padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer;">
                            🗑️ Delete
                        </button>
                    </div>
                </div>
        ...
        container.innerHTML = `<p style="color: #ef4444;">Error loading events. Check console.</p>`;
    });
}
                ...
                if (!snapshot3.empty) {
                    ...
                    container.innerHTML = `
                        <div style="background: #fef3c7; padding: 15px; border-radius: 8px; color: #92400e;">
                            ...
                            <p>Create a 'members' collection in Firebase with: name, email, position</p>
                        </div>
                    `;
                }
            });
...
window.displayMembers = function(snapshot, container) {
    ...
    html += "<tr style='background: #f3f4f6;'><th style='padding:12px; text-align:left; border-bottom: 2px solid #ddd;'>Name</th><th style='padding:12px; text-align:left; border-bottom: 2px solid #ddd;'>Email</th><th style='padding:12px; text-align:left; border-bottom: 2px solid #ddd;'>Position</th></tr>";
